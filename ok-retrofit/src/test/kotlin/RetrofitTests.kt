 ...
 * response -> checkSuccessfull(response) exists to cover all kinds of HTTP scenarios. Additional endpoints are being considered.
...
interface ApiService {
    ...
    @GET("/wrong")
    fun fails(): Single<Response<String>>
}
 ...
 *
 * Assertions like..
 *    response -> checkSuccessfull(response) should have schema "schemas/ip.json"
 ...
 */
class RetrofitTests : FeatureSpec() { init {


    val httpbin = retrofit.create(ApiService::class.java)

    feature("GET /user-agent") {
        rxScenario("verify schema", httpbin.userAgent()) { response ->
            val body = checkSuccessfull(response)
            body should have schema "schemas/user-agent.json"
        }
    }

    feature("fails should fail") {
        rxScenario("fails", httpbin.fails()) { response ->
            shouldThrow<AssertionError> { checkSuccessfull(response) }
        }
    }

    feature("GET /ip") {
        rxScenario("verify schema", httpbin.ip()) { response ->
            val body = checkSuccessfull(response)
            body should have schema "schemas/ip.json"
        }
    }

    feature("GET /get") {
        val args = hashMapOf("id" to "1", "gender" to "MALE")
        rxScenario("verify schema", httpbin.getMap(args)) { response ->
            checkSuccessfull(response) should have schema "schemas/get.json"
        }

        rxScenario("verify schema", httpbin.getParams("1", "MALE")) { response ->
            checkSuccessfull(response)should have schema "schemas/get.json"
        }
    }
    ...
    feature("GET /headers") {
        ...
        rxScenario("headersMap", httpbin.headersMap(headers)) { response ->
            val body = checkSuccessfull(response)
            body should have schema "schemas/headers.json"
            body.childObject("headers").keys() should containAll(*keys)
        }

        rxScenario("headersParam", httpbin.headersParam("application/json", "okhttp3")) { response ->
            val body = checkSuccessfull(response)
            body should have schema "schemas/headers.json"
            body.childObject("headers").keys() should containAll(*keys)
        }
    ...
    feature("POST /post") {
        ...
        rxScenario("/post x-www-form-urlencoded", httpbin.postUrlEncoded(postData)) { response ->
            val body = checkSuccessfull(response)
            body should have schema "schemas/post.json"
            body.childObject("form").keys() should containAll(*keys)
        }

        rxScenario("/post multipart form-data", httpbin.postFormData("cheese", "small", "comments", "13h45")) { response ->
            val body = checkSuccessfull(response)
            body should have schema "schemas/post.json"
            body.childObject("form").keys() should containAll(*keys)
        }

        rxScenario("/post JSON body", httpbin.postJson(postData)) { response ->
            val body = checkSuccessfull(response)
            body should have schema "schemas/post.json"
            body.childObject("json").keys() should containAll(*keys)
        }
    }
    ...
    feature("PUT /put") {
        ...
        rxScenario("/put x-www-form-urlencoded", httpbin.putUrlEncoded(putData)) { response ->
            checkSuccessfull(response) should have schema "schemas/post.json"
        }

        rxScenario("/put JSON body", httpbin.putJson(putData)) { response ->
            checkSuccessfull(response) should have schema "schemas/post.json"
        }
    }
